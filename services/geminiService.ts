...
import { GEMINI_TEXT_MODEL, TRAVEL_TIERS } from '../constants';
...
const parseItineraryFromString = (text: string, formData: TripFormData): Omit<GeneratedItinerary, 'budget'> => {
...
export const generateItinerary = async (formData: TripFormData): Promise<GeneratedItinerary> => {
  ...
  const prompt = `
    ...
    Day 1 ([Actual Date based on Start Date, e.g., YYYY-MM-DD]):
  ...
  try {
    ...
    const parsedData = parseItineraryFromString(text, formData);
...
export const generatePackingList = async (destinations: string[], startDate: string, endDate: string, travelType: string): Promise<string[]> => {
  ...
  const prompt = `
    ...
    For example: ["Sunscreen", "Passport", "Phone Charger", "Hiking Boots"]
    ...
    try {
      ...
      if (Array.isArray(parsedData) && parsedData.every(item => typeof item === 'string')) {
        return parsedData as string[];
         ...
         const fallbackList = jsonStr.split('\n').map(s => s.replace(/^- |^\* |^"|"|,$/g, '').trim()).filter(s => s.length > 0 && !s.match(/^\[|\]$/));
      ...
      const fallbackList = response.text.split('\n').map(s => s.replace(/^- |^\* |^"|"|,$/g, '').trim()).filter(s => s.length > 0 && !s.match(/^\[|\]$/));
...
export const generateTravelNotificationContent = async (
  ...
  homeAddress?: string,
  tripOccasion?: string,
  daysUntilTrip?: number // Added daysUntilTrip
): Promise<string> => {
  if (!ai) {
    console.warn("Gemini API not initialized. Returning mock notification content.");
    let mockMessage = "Mock: Friendly travel reminder!";
    if (notificationType === 'upcoming_trip_general_reminder') {
      if (daysUntilTrip === 1) mockMessage = `Mock: Your awesome trip to ${destination} is TOMORROW! 🤩 Get packing!`;
      else if (daysUntilTrip === 7) mockMessage = `Mock: Just 1 week until ${destination}! 🗓️ The countdown is ON!`;
      else mockMessage = `Mock: Only ${daysUntilTrip} days left until your trip to ${destination}! 🎉`;
    } else if (notificationType === 'departure_cab') {
      mockMessage = `Mock: Time to head to ${destination}! ✈️ Don't forget your cab from ${homeAddress || 'your current location'} to the airport. Tap to book!`;
    } else if (notificationType === 'arrival_cab') {
      mockMessage = `Mock: Welcome back from ${destination}! 🏡 Need a ride home from the airport to ${homeAddress || 'your destination'}? We can help!`;
    }
    return mockMessage;
  }
  ...
  switch (notificationType) {
    case 'upcoming_trip_general_reminder':
      if (daysUntilTrip === 1) {
        promptSpecifics = `Their fabulous trip to ${destination} is TOMORROW! Make it sound super exciting and urgent (in a fun way). Example: "Hold onto your hats, globetrotter! 🎩 Your grand escape to ${destination} is TOMORROW! Adventure (and maybe some chaos) awaits! 🚀"`;
      } else if (daysUntilTrip === 7) {
        promptSpecifics = `Their amazing trip to ${destination} is just 1 week away! Build anticipation. ${tripOccasion ? `This trip is for their ${tripOccasion}.` : ''} Example: "Hey jet-setter! ✨ Just 7 sleeps until your Parisian adventure! Time to start dreaming of croissants and chic vibes. 😉"`;
      } else {
        promptSpecifics = `Their wonderful trip to ${destination} is ${daysUntilTrip} days away. Give them a fun countdown reminder. ${tripOccasion ? `Mention their ${tripOccasion} if relevant.` : ''} Example: "Good news, superstar! Only ${daysUntilTrip} more days until you're sipping cocktails in ${destination}. The glam life is calling! 💅"`;
      }
      break;
    case 'departure_cab':
      promptSpecifics = `Their flight to ${destination} is today! Remind them to book a cab to the airport from ${homeAddress ? homeAddress : 'their current spot'} and mention they can book it via the app. Example: "It's D-Day for ${destination}! ✨ Your throne (aka plane seat) awaits. Need a royal chariot to the airport from ${homeAddress || 'your current launchpad'}? Tap here, darling, and we'll sort you out! 💅"`;
      break;
    case 'arrival_cab':
      promptSpecifics = `They are returning today from their trip to ${destination}. Remind them to book a cab from the airport to ${homeAddress ? homeAddress : 'their final destination'} through the app. Example: "Welcome back, superstar! Hope ${destination} treated you like royalty. ✨ Now, let's glide you home from the airport to ${homeAddress || 'your castle'} without a fuss. Tap for a comfy cab ride! 🏰"`;
      break;
    default:
  ...
  try {
    ...
    return `Friendly reminder for your trip to ${destination}! Just ${daysUntilTrip || 'a few'} days left.`;
  }
};
...
export const fetchPlaceSuggestions = async (destination: string): Promise<PlaceSuggestion[]> => {
  if (!ai) {
    console.warn("Gemini API not initialized. Returning mock place suggestions.");
    await new Promise(resolve => setTimeout(resolve, 1000));
    return [
      { id: "mock-place-1", name: `Mock Restaurant in ${destination}`, category: "Restaurant", description: "A lovely place for mock food." },
      { id: "mock-place-2", name: `Mock Museum of ${destination}`, category: "Museum", description: "Explore mock history." },
      { id: "mock-place-3", name: `Mock Park in ${destination}`, category: "Park", description: "Relax and enjoy mock nature." },
    ];
  }

  const prompt = `
    Suggest 5-7 interesting places (restaurants, cafes, tourist attractions, parks, etc.) in or very near ${destination}.
    For each place, provide:
    - A unique "id" (string, e.g., "place-123").
    - "name" (string, the name of the place).
    - "category" (string, e.g., "Restaurant", "Museum", "Park", "Cafe", "Landmark").
    - "description" (string, a brief 1-2 sentence description).
    - Optionally, "imageUrl" (string, a publicly accessible direct image URL if available, otherwise omit or null).
    - Optionally, if you can infer approximate "latitude" and "longitude" (numbers), include them as "location": {"lat": number, "lng": number}. Otherwise, omit "location".

    Format the response as a JSON array of these objects.
    Example:
    [
      {
        "id": "place-001",
        "name": "The Grand Cafe",
        "category": "Cafe",
        "description": "A charming cafe known for its excellent coffee and pastries. Perfect for a morning break.",
        "imageUrl": "https://images.unsplash.com/photo-example-cafe",
      ...
      },
      {
        ...
        "category": "Museum",
        "description": "Discover the rich history of the city through interactive exhibits."
      }
    ]
    Output ONLY the JSON array. Do not include any other text, markdown, or explanations.
  `;
  try {
    ...
    try {
      ...
      if (Array.isArray(parsedData) && parsedData.every(item =>
        ...
        (typeof item.imageUrl === 'string' || typeof item.imageUrl === 'undefined' || item.imageUrl === null) &&
        (typeof item.location === 'object' || typeof item.location === 'undefined') &&
        (typeof item.location?.lat === 'number' || typeof item.location?.lat === 'undefined') &&
        (typeof item.location?.lng === 'number' || typeof item.location?.lng === 'undefined')
      )) {
        ...
        console.warn("Place suggestions from AI were not in the expected JSON array format. Received:", parsedData, "Cleaned string:", jsonStr);
        throw new Error("AI returned data in an unexpected format for place suggestions.");
      }
    } catch (e) {
      console.error("Failed to parse JSON response for place suggestions:", e, "Raw response:", response.text, "Cleaned string:", jsonStr);
      throw new Error("Failed to parse place suggestions from AI.");
    }
  } catch (error) {
    console.error(`Error fetching place suggestions for ${destination} with Gemini:`, error);
    if (error instanceof Error && error.message.includes("API key not valid")) {
      throw new Error(`Invalid API Key for place suggestions. Please check your API_KEY.`);
    }
    throw new Error(error instanceof Error ? error.message : `Failed to fetch place suggestions for ${destination}.`);
  }
};