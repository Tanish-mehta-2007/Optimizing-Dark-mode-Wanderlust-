...
import { ExploreDestination, AppView } from '../../types';
...
const ExploreView: React.FC<ExploreViewProps> = ({ 
  ...
  return (
    <div className="flex-1 flex flex-col bg-emerald-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 py-6 sm:py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex-1 flex flex-col">
        {/* Back Button */}
        <button
          onClick={onNavigateBack}
          className="mb-4 sm:mb-6 inline-flex items-center px-3 py-1.5 text-xs sm:text-sm font-medium text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-slate-700 hover:bg-emerald-200 dark:hover:bg-slate-600 rounded-lg shadow-sm transition-colors button-interactive"
        >
        ...
        <div className="mb-6 sm:mb-10 flex items-center justify-start space-x-1.5 sm:space-x-2 overflow-x-auto pb-2 custom-scrollbar">
          {CONTINENTS.map(continent => (
            <button
              ...
              className={`px-3 py-1.5 text-[11px] sm:text-xs font-medium rounded-full transition-colors whitespace-nowrap button-interactive
        ...
        {filteredDestinations.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 flex-grow"> {/* Added flex-grow */}
                  ...
                  <div className={`relative w-full aspect-[4/3] rounded-xl shadow-lg transition-transform duration-700 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                    ...
                    <div className="absolute inset-0 w-full h-full backface-hidden bg-emerald-600 dark:bg-emerald-700 rounded-xl overflow-y-auto border border-emerald-500 dark:border-emerald-600 transform rotate-y-180 p-3 sm:p-4 flex flex-col justify-between text-white">
                      ...
                      <button
                        ...
                        className="mt-2 w-full inline-flex items-center justify-center px-3 py-1.5 bg-white/20 hover:bg-white/30 text-white text-xs font-semibold rounded-md transition-colors group/button button-interactive"
          ...
          <div className="text-center py-12 flex-grow flex flex-col justify-center items-center"> {/* Added flex for centering empty state */}
        ...
        {/* Footer - Only shown if not part of a larger scrolling page that has GlobalFooter */}
        {/* This footer might be redundant if GlobalFooter is always shown */}
        <footer className="mt-auto pt-6 sm:pt-8 border-t border-emerald-200 dark:border-slate-700 text-center">
