...
const SignupPage: React.FC<SignupPageProps> = ({ onSignupSuccess, navigateToLogin }) => {
  ...
  const { signup, socialLogin } = useContext<AuthContextType>(AuthContext);
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-4 page-transition-enter">
      <div className="w-full max-w-sm sm:max-w-md bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700">
        ...
        <div className="space-y-2.5 sm:space-y-3">
          <button onClick={() => handleSocialLogin('google')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-white dark:bg-slate-700 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600/70 disabled:opacity-70 transition-colors button-interactive"> <GoogleIcon /> Sign up with Google </button>
          <button onClick={() => handleSocialLogin('microsoft')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-white dark:bg-slate-700 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600/70 disabled:opacity-70 transition-colors button-interactive"> <MicrosoftIcon /> Sign up with Microsoft </button>
          <button onClick={() => handleSocialLogin('apple')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-black text-xs sm:text-sm font-medium text-white hover:bg-slate-800 dark:bg-white dark:text-black dark:hover:bg-slate-200 disabled:opacity-70 transition-colors button-interactive"> <AppleIcon /> Sign up with Apple </button>
        </div>
        
        <p className="mt-6 sm:mt-8 text-center text-xs sm:text-sm text-slate-600 dark:text-slate-400">
          Already have an account?{' '}
          <button onClick={navigateToLogin} className="font-medium text-brand hover:text-brand-dark dark:text-brand-light dark:hover:text-blue-400 underline interactive-element button-interactive"> Log in </button>
        </p>
      </div>
    </div>
  );
