...
const HamburgerIcon = () => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-1.5-5.25h16.5" /></svg>;
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-850/95 backdrop-blur-sm px-4 sm:px-6 md:px-10 py-3 h-[var(--header-height)]">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div 
                className="flex items-center gap-2 sm:gap-3 text-slate-800 dark:text-slate-100 cursor-pointer interactive-element nav-item-interactive rounded-md p-1 -ml-1 button-interactive"
        ...
        <nav className="hidden md:flex items-center gap-1 sm:gap-2">
            {desktopNavLinks.map(link => {
                const isActive = currentView === link.view || 
                                (link.view === AppView.STRUCTURED_ITINERARY_BUILDER && [AppView.TRANSPORTATION_MODE_SELECTION, AppView.ITINERARY, AppView.FLIGHT_BOOKING, AppView.HOTEL_BOOKING, AppView.CAR_BOOKING, AppView.TRAIN_BOOKING, AppView.BUS_BOOKING, AppView.PAYMENT_PAGE, AppView.PACKING_LIST, AppView.BUDGET_DISPLAY].includes(currentView));
                return (
                    <button 
                        key={link.labelKey} 
                        onClick={link.action}
