            ...
            <div className="flex flex-wrap gap-2 border-b border-slate-200 dark:border-slate-700 pb-3">
              {tripCities.map((city, index) => (
                <button
                  ...
                  className={`px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium rounded-md transition-colors button-interactive
        ...
        <form onSubmit={handleSearch} className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 space-y-4">
          <div ref={destinationInputContainerRef} className="relative">
              ...
              {showDestinationAutocomplete && destinationAutocompleteSuggestions.length > 0 && (
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md shadow-lg max-h-60 overflow-y-auto"><ul className="py-1">{destinationAutocompleteSuggestions.map(loc => <li key={loc} onMouseDown={() => handleDestinationAutocompleteSelect(loc)} className="px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-blue-600 cursor-pointer flex items-center list-item-interactive"><LocationPinIcon /> {loc}</li>)}</ul></div>
              )}
          </div>
          ...
          <button type="submit" disabled={isSearching || !searchCriteria.destination} className="w-full flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg shadow-md transition-colors duration-200 disabled:bg-slate-400 dark:disabled:bg-slate-500 button-interactive">
