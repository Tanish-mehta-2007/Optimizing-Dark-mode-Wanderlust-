...
const DragHandleIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
    <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 9h16.5m-1.5-6.75h16.5" />
  </svg>
);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 p-3 sm:p-4 bg-slate-100 dark:bg-slate-850 rounded-b-lg -mt-px">
      ...
      <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-2 pt-2">
        <button type="button" onClick={onCancel} disabled={isSaving} className="w-full sm:w-auto px-3 py-1.5 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 rounded-md transition button-interactive">Cancel</button>
        <button type="submit" disabled={isSaving} className="w-full sm:w-auto px-3 py-1.5 text-xs sm:text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 rounded-md transition disabled:opacity-70 button-interactive">
...
export const ItineraryDisplay: React.FC<ItineraryDisplayProps> = ({ onNext, onBack }) => {
  const { 
    currentTrip, 
    addItineraryEvent, 
  ...
  // const [activeEventIdentifier, setActiveEventIdentifier] = useState<string | null>(null); // Map interaction removed
  const itineraryScrollRef = useRef<HTMLDivElement>(null);
  // const eventRefs = useRef<Map<string, HTMLElement | null>>(new Map()); // Map interaction removed
  ...
  // Removed useEffect related to map and activeEventIdentifier
  ...
  if (!currentTrip || !processedItinerary) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center p-6 sm:p-8 bg-white dark:bg-slate-800 rounded-xl shadow-xl">
        ...
        <button onClick={onBack} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-6 rounded-lg shadow-md transition duration-300 transform hover:scale-105 button-interactive">Back to Planner</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col bg-white dark:bg-slate-900 p-3 sm:p-4 md:p-6 rounded-xl shadow-xl max-w-full mx-auto text-slate-700 dark:text-slate-200 overflow-y-auto custom-scrollbar">
      ...
      {/* Map Placeholder */}
      <div className="w-full h-40 sm:h-56 md:h-64 rounded-lg shadow-md overflow-hidden border border-slate-200 dark:border-slate-700 mb-6">
      ...
      <div ref={itineraryScrollRef} className="overflow-y-auto space-y-1 pr-0 sm:pr-1 custom-scrollbar">
          ...
          <div key={day.day + dayIndex} className="mb-4 sm:mb-6">
            <div className="sticky top-0 bg-white dark:bg-slate-900 py-2 sm:py-3 z-10 border-b-2 border-blue-300 dark:border-blue-600 flex justify-between items-center mb-2 sm:mb-3">
              ...
              <button 
                ...
                className="flex items-center text-xs sm:text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium rounded-md bg-blue-50 dark:bg-blue-800/60 hover:bg-blue-100 dark:hover:bg-blue-700/80 transition p-1 sm:p-1.5 button-interactive"
                  ...
                  <React.Fragment key={event.identifier}>
                    <div 
                      ...
                      className={`bg-slate-50 dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-300 ease-in-out
                                  ${editingEventIdentifier === event.identifier ? 'ring-2 ring-offset-1 ring-offset-white dark:ring-offset-slate-900 ring-sky-500 dark:ring-sky-400 shadow-xl' : 'hover:shadow-lg'}
                            ...
                            <div className="flex justify-between items-start mb-1.5">
                              <div className="flex-grow">
                                ...
                                <div className="flex space-x-1 shrink-0 ml-1.5">
                                    {canEditEvent && (
                                      <button onClick={() => handleEditEvent(event.identifier)} className="p-1 text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors button-interactive" title="Edit Event"><EditIcon /></button>
                                    )}
                                    <button onClick={() => handleDeleteEvent(dayIndex, event.identifier)} className="p-1 text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-500 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors button-interactive" title="Delete Event"><DeleteIcon /></button>
                                    <button className="p-1 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-grab button-interactive" title="Drag to reorder"><DragHandleIcon /></button>
                                </div>
                            </div>
      ...
      <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row sm:justify-between items-center gap-3">
        <button onClick={onBack} className="w-full sm:w-auto bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold py-2 sm:py-2.5 px-4 sm:px-5 rounded-lg shadow-sm transition-colors duration-200 button-interactive">Back</button>
        <div className="flex items-center gap-2">
            <button 
                onClick={undoItineraryChange} 
                disabled={!canUndoItinerary()}
                className="flex items-center px-3 py-1.5 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium rounded-md bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 text-slate-700 dark:text-slate-200 disabled:opacity-50 disabled:cursor-not-allowed button-interactive"
        ...
        <button onClick={onNext} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 sm:py-2.5 px-4 sm:px-5 rounded-lg shadow-md transition-colors duration-200 transform hover:scale-105 button-interactive">Proceed to Bookings</button>
      </div>
      <style>{`
...
// export default ItineraryDisplay; // Removed default export
