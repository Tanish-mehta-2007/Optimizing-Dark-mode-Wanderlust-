...
import { FlightBooking, FlightClass, FlightLegCriteria, AppView } from '../../types'; // Added AppView
import { fetchFlightOptions } from '../services/mockBookingService';
import { FLIGHT_CLASSES, POPULAR_DESTINATIONS } from '../constants';
import { AppContext } from '../App'; // Added AppContext import
...
export const FlightBookingPage: React.FC<FlightBookingPageProps> = ({ onNext, onBack, isStandaloneMode = false }) => {
  ...
  const handleActualBack = () => {
    if (isStandaloneMode) {
      ...
      if (navigateTo) {
        navigateTo(AppView.TRANSPORTATION_MODE_SELECTION);
      } else {
        onBack(); 
      }
    }
    ...
    <div className="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 px-4 py-6 sm:p-6">
        ...
        <form onSubmit={handleSearch} ref={flightBookingFormRef} className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 space-y-4">
          {searchCriteria.legs.map((leg, index) => (
            <div key={leg.legId} className="p-3 sm:p-4 border border-slate-200 dark:border-slate-700 rounded-lg space-y-3 bg-slate-50 dark:bg-slate-800/50 relative">
              <div className="flex justify-between items-center mb-2">
                  ...
                  {searchCriteria.legs.length > 1 && <button type="button" onClick={() => handleRemoveLeg(index)} className="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 p-1 rounded-full hover:bg-red-100/50 dark:hover:bg-red-800/30 transition-colors button-interactive"><MinusCircleIcon /></button>}
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="relative">
                      ...
                      {activeLegInput?.legId === leg.legId && activeLegInput.inputType === 'origin' && showAutocomplete && autocompleteSuggestions.length > 0 && (
                          <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md shadow-lg max-h-48 overflow-y-auto"><ul className="py-1">{autocompleteSuggestions.map(loc => <li key={loc} onMouseDown={() => handleAutocompleteSelect(index, 'origin', loc)} className="px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-blue-600 cursor-pointer flex items-center list-item-interactive"><LocationPinIcon /> {loc}</li>)}</ul></div>
                      )}
                  </div>
                  <div className="relative">
                      ...
                      {activeLegInput?.legId === leg.legId && activeLegInput.inputType === 'destination' && showAutocomplete && autocompleteSuggestions.length > 0 && (
                          <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md shadow-lg max-h-48 overflow-y-auto"><ul className="py-1">{autocompleteSuggestions.map(loc => <li key={loc} onMouseDown={() => handleAutocompleteSelect(index, 'destination', loc)} className="px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-blue-600 cursor-pointer flex items-center list-item-interactive"><LocationPinIcon /> {loc}</li>)}</ul></div>
                      )}
                  </div>
                  <div>
          ...
          {!isStandaloneMode && searchCriteria.legs.length < 5 && <button type="button" onClick={handleAddLeg} className="text-sm text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium flex items-center button-interactive"><PlusCircleIcon />Add Another Leg</button>}
          ...
          <button type="submit" disabled={Object.values(isSearching).some(s => s)} className="w-full flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg shadow-md transition-colors duration-200 disabled:bg-slate-400 dark:disabled:bg-slate-500 button-interactive">
        ...
        <div className="mt-8 sm:mt-10 pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-col-reverse sm:flex-row sm:justify-between gap-3">
          <button onClick={handleActualBack} className="w-full sm:w-auto bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg shadow-sm transition-colors button-interactive">
