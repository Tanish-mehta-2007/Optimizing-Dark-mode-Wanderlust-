...
// Removed: import { fetchNearbyPlaces } from '../services/googleMapsApiService.ts';
...
const NearbySuggestionsPage: React.FC<NearbySuggestionsPageProps> = ({ navigateTo }) => {
  ...
  const fetchUserLocationAndThenSuggestions = useCallback(async () => {
    ...
    try {
      ...
      // fetchNearbySuggestionsFromGoogle(location); // Google Maps API call removed
      setError("Nearby place suggestions are temporarily unavailable as the map service has changed."); // Set error
      setIsLoadingSuggestions(false); // Ensure loading state is false
    } catch (err) {
  ...
  // fetchNearbySuggestionsFromGoogle function is no longer needed as the API is stubbed
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row bg-slate-100 dark:bg-slate-950 overflow-hidden">
      {/* Left Column */}
      <div className="w-full md:w-[420px] lg:w-[450px] bg-white dark:bg-slate-900 flex flex-col border-r border-slate-200 dark:border-slate-800">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800">
            <button 
                ...
                className="mb-3 inline-flex items-center text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium group button-interactive"
            >
                <BackArrowIcon /> Back
            </button>
            <div className="relative">
                ...
                />
                 <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                    <button onClick={handleRefresh} disabled={isLoadingLocation || isLoadingSuggestions} className="p-1 text-slate-500 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-300 transition-colors button-interactive" title="Refresh suggestions">
          ...
          <div className="flex space-x-1">
            {(["All", "Eat & Drink", "See & Do"] as FilterTab[]).map(tab => (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-md transition-colors button-interactive
                  ${activeFilterTab === tab
          ...
          <div className="flex-grow overflow-y-auto p-4 space-y-3 custom-scrollbar-nearby">
            ...
            {filteredSuggestions.map(place => (
              <div
                ...
                className={`p-3 bg-white dark:bg-slate-850 rounded-lg border cursor-pointer transition-all duration-200 ease-in-out flex items-center space-x-3 card-interactive-lift
