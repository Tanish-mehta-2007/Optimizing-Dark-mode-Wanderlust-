...
import { AppContext } from '../App'; // Import AppContext for navigation
...
const NotificationTypeIcon: React.FC<{ type: NotificationItem['type'] }> = ({ type }) => {
  ...
  switch (type) {
    case 'alert':
      iconElement = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-full h-full"><path fillRule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" /></svg>;
      colorClasses = "text-red-500 dark:text-red-400";
      break;
    case 'info':
      iconElement = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-full h-full"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clipRule="evenodd" /></svg>;
      colorClasses = "text-blue-500 dark:text-blue-400";
      break;
    case 'reminder':
      iconElement = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-full h-full"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm.75-13a.75.75 0 00-1.5 0v5c0 .414.336.75.75.75h4a.75.75 0 000-1.5h-3.25V5z" clipRule="evenodd" /></svg>;
      colorClasses = "text-amber-500 dark:text-amber-400";
      break;
    case 'update':
      iconElement = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-full h-full"><path d="M10.75 2.75a.75.75 0 00-1.5 0v8.614L6.295 8.235a.75.75 0 10-1.09 1.03l4.25 4.5a.75.75 0 001.09 0l4.25-4.5a.75.75 0 00-1.09-1.03l-2.955 3.129V2.75z" /><path d="M3.5 12.75a.75.75 0 00-1.5 0v2.5A2.75 2.75 0 004.75 18h10.5A2.75 2.75 0 0018 15.25v-2.5a.75.75 0 00-1.5 0v2.5c0 .69-.56 1.25-1.25 1.25H4.75c-.69 0-1.25-.56-1.25-1.25v-2.5z" /></svg>;
      colorClasses = "text-emerald-500 dark:text-emerald-400";
      break;
    case 'system':
       iconElement = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-full h-full"><path fillRule="evenodd" d="M11.09 3.562A4.5 4.5 0 105.59 9.062a4.5 4.5 0 005.5 0zM10 12.5a.75.75 0 100-1.5.75.75 0 000 1.5z" clipRule="evenodd" /><path d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.125 5.462a3 3 0 101.75 5.075V12.5a.75.75 0 00-1.5 0V10.537A3 3 0 009.125 5.462z" /></svg>;
      colorClasses = "text-purple-500 dark:text-purple-400";
      break;
    default:
      iconElement = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-full h-full"><path d="M10 2a.75.75 0 01.75.75v1.5a.75.75 0 01-1.5 0v-1.5A.75.75 0 0110 2zM10 15a.75.75 0 01.75.75v1.5a.75.75 0 01-1.5 0v-1.5A.75.75 0 0110 15zM10 7a3 3 0 100 6 3 3 0 000-6zM15.657 5.404a.75.75 0 10-1.06-1.06l-1.061 1.06a.75.75 0 001.06 1.06l1.06-1.06zM5.404 15.657a.75.75 0 10-1.06-1.06l-1.06 1.06a.75.75 0 101.06 1.06l1.06-1.06zM16.717 10a.75.75 0 01-1.5 0h-1.5a.75.75 0 010-1.5h1.5a.75.75 0 011.5 0zM4.283 10a.75.75 0 01-1.5 0H1.25a.75.75 0 010-1.5h1.533a.75.75 0 011.5 0zM14.596 15.657a.75.75 0 001.06-1.06l-1.06-1.061a.75.75 0 10-1.06 1.06l1.06 1.06zM4.343 5.404a.75.75 0 001.06-1.06l-1.06-1.06a.75.75 0 10-1.061 1.06l1.061 1.06z" /></svg>;
  }
  return iconElement ? <div className={`w-5 h-5 sm:w-6 sm:h-6 ${colorClasses}`}>{iconElement}</div> : null;
};
...
export const NotificationsPage: React.FC<NotificationsPageProps> = ({ onNavigateBack }) => {
  ...
  const handleNotificationClick = (notification: NotificationItem) => {
    if (notification.link && navigateTo) {
      navigateTo(notification.link.view, notification.link.params);
    }
  ...
  if (!currentUser) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center py-16 sm:py-20 px-4 bg-slate-50 dark:bg-slate-900">
        <BellIconHeader />
        <h2 className="text-xl sm:text-2xl font-bold text-slate-700 dark:text-slate-200 mt-4">Access Denied</h2>
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 dark:bg-slate-900 overflow-y-auto custom-scrollbar">
      <div className="max-w-3xl mx-auto px-4 py-6 sm:px-6 sm:py-8 lg:px-8 flex-1">
        <button
        ...
        {userNotifications.length === 0 ? (
          ...
          <div className="space-y-3 sm:space-y-4">
            {userNotifications.map(notification => (
              <div
                key={notification.id}
                className={`p-3 sm:p-4 rounded-lg shadow-md border transition-all duration-300 flex items-start space-x-2 sm:space-x-3 card-interactive-lift
              ...
              >
                ...
                <div className="flex-shrink-0 flex flex-col sm:flex-row items-center gap-1 sm:gap-2">
                  ...
                  <button
                    onClick={(e) => {e.stopPropagation(); handleClearNotification(notification.id);}}
