...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, navigateToSignup }) => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 page-transition-enter">
      <div className="w-full max-w-sm sm:max-w-md bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700">
        <div className="text-center mb-6 sm:mb-8">
          <div className="inline-flex items-center justify-center mb-2 sm:mb-3">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-brand-dark dark:text-brand-light">Wanderlust Login</h1>
          </div>
          <p className="text-slate-600 dark:text-slate-300 text-sm sm:text-base">Welcome back, traveler!</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
          <div>
            <label htmlFor="email" className="block text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200">Email address</label>
          ...
          <div>
            <label htmlFor="password"className="block text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200">Password</label>
          ...
          <div>
            <button type="submit" disabled={isLoading}
              className="w-full flex justify-center py-2.5 sm:py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm sm:text-base font-medium text-white bg-brand hover:bg-brand-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-light dark:focus:ring-offset-slate-800 disabled:opacity-70 transition-colors button-interactive">
              {isLoading ? 'Logging in...' : 'Log In'}
            </button>
          </div>
        </form>

        <div className="my-5 sm:my-6"><div className="relative"><div className="absolute inset-0 flex items-center" aria-hidden="true"><div className="w-full border-t border-slate-300 dark:border-slate-600" /></div><div className="relative flex justify-center text-xs sm:text-sm"><span className="px-2 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400">Or continue with</span></div></div></div>

        <div className="space-y-2.5 sm:space-y-3">
          <button onClick={() => handleSocialLogin('google')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-white dark:bg-slate-700 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600/70 disabled:opacity-70 transition-colors button-interactive"> <GoogleIcon /> Sign in with Google </button>
          <button onClick={() => handleSocialLogin('microsoft')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-white dark:bg-slate-700 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600/70 disabled:opacity-70 transition-colors button-interactive"> <MicrosoftIcon /> Sign in with Microsoft </button>
          <button onClick={() => handleSocialLogin('apple')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-black text-xs sm:text-sm font-medium text-white hover:bg-slate-800 dark:bg-white dark:text-black dark:hover:bg-slate-200 disabled:opacity-70 transition-colors button-interactive"> <AppleIcon /> Sign in with Apple </button>
        </div>

        <p className="mt-6 sm:mt-8 text-center text-xs sm:text-sm text-slate-600 dark:text-slate-400">
          Don't have an account?{' '}
          <button onClick={navigateToSignup} className="font-medium text-brand hover:text-brand-dark dark:text-brand-light dark:hover:text-blue-400 underline interactive-element button-interactive"> Sign up here </button>
        </p>
      </div>
    </div>
  );
