...
export const ProfilePictureManager: React.FC = () => {
  const { currentUser, updateCurrentUserProfileImage } = useContext(AuthContext);
    ...
    <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 w-full">
      ...
      <div className="flex flex-col items-center space-y-4 sm:space-y-6">
        ...
        <button
          ...
          className="px-4 py-2 sm:px-6 sm:py-2.5 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-medium rounded-lg shadow-sm border border-slate-300 dark:border-slate-500 transition-colors text-xs sm:text-sm"
          disabled={isLoading}
        >
        ...
        {selectedFile && previewUrl && (
          <button
            ...
            disabled={isLoading}
            className="px-4 py-2 sm:px-6 sm:py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg shadow-md transition-colors disabled:opacity-70 flex items-center justify-center text-xs sm:text-sm"
          >
