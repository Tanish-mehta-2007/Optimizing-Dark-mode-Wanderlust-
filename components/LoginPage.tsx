...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, navigateToSignup }) => {
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-100 dark:bg-slate-950 p-4 page-transition-enter">
      <div className="w-full max-w-sm sm:max-w-md bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700">
        ...
        <div className="space-y-2.5 sm:space-y-3">
          <button onClick={() => handleSocialLogin('google')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-white dark:bg-slate-700 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600/70 disabled:opacity-70 transition-colors"> <GoogleIcon /> Sign in with Google </button>
          <button onClick={() => handleSocialLogin('microsoft')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-white dark:bg-slate-700 text-xs sm:text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600/70 disabled:opacity-70 transition-colors"> <MicrosoftIcon /> Sign in with Microsoft </button>
          <button onClick={() => handleSocialLogin('apple')} disabled={isLoading} className="w-full inline-flex items-center justify-center px-4 py-2 sm:py-2.5 border border-slate-300 dark:border-slate-600 rounded-lg shadow-sm bg-black text-xs sm:text-sm font-medium text-white hover:bg-slate-800 dark:bg-white dark:text-black dark:hover:bg-slate-200 disabled:opacity-70 transition-colors"> <AppleIcon /> Sign in with Apple </button>
        </div>

        <p className="mt-6 sm:mt-8 text-center text-xs sm:text-sm text-slate-600 dark:text-slate-400">
          Don't have an account?{' '}
          <button onClick={navigateToSignup} className="font-medium text-brand hover:text-brand-dark dark:text-brand-light dark:hover:text-blue-400 underline"> Sign up here </button>
        </p>
      </div>
    </div>
  );
