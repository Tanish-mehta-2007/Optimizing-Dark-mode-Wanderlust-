...
// import ItineraryMap from './ItineraryMap'; // Removed ItineraryMap import
...
const StructuredItineraryBuilderPage: React.FC<StructuredItineraryBuilderPageProps> = ({ onItineraryGenerated }) => {
  ...
  // useEffect(() => { // Removed: This useEffect was for updating mapLocations
  ...
  // const selectedTierDetails = TRAVEL_TIERS.find(t => t.id === formData.travelTier); // Removed as summary is removed
  ...
  return (
    <div className="h-screen-minus-header bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-100">
      <div className="h-full overflow-y-auto p-4 sm:p-6 custom-scrollbar"> {/* Removed md:w-[55%] lg:w-[60%] */}
                        ...
                        <div>
                            ...
                            {formData.destinations.map((destination, index) => (
                                <div key={index} className="flex items-center gap-2 mb-2.5" ref={el => { destinationInputContainerRefs.current[index] = el; }}>
                                    ...
                                    {activeDestinationIndex === index && filteredDestinationSuggestions.length > 0 && (<div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md shadow-lg max-h-40 overflow-y-auto"><ul className="py-1">{filteredDestinationSuggestions.map(dest => <li key={`dest-${index}-${dest}`} onMouseDown={() => handleDestinationAutocompleteSelect(index, dest)} className="px-4 py-2.5 text-sm hover:bg-blue-50 dark:hover:bg-blue-600/50 cursor-pointer flex items-center"><LocationPinIcon/>{dest}</li>)}</ul></div>)}</div>
                                    {formData.destinations.length > 1 && <button type="button" onClick={() => handleRemoveDestination(index)} className="p-2 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 rounded-full hover:bg-red-100/50 dark:hover:bg-red-800/30 transition-colors"><MinusCircleIcon/></button>}
                                </div>))}
                            {formData.destinations.length < 5 && <button type="button" onClick={handleAddDestination} className="mt-1 text-sm text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium flex items-center"><PlusCircleIcon/>Add another city</button>}
                        </div>
