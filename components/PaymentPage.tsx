...
import { FlightBooking, HotelBooking, CarRental, AppView, TrainBooking } from '../types'; // Added TrainBooking and AppView
...
const PaymentPage: React.FC<PaymentPageProps> = ({ onNext, onBack }) => {
  const { currentTrip, updateFlightBooking, updateHotelBooking, updateCarRental, updateTrainBooking } = useContext(TripContext); 
  ...
  const itemsToPay: (FlightBooking | HotelBooking | CarRental | TrainBooking)[] = [ 
    ...flightsToPay,
    ...hotelsToPay,
    ...(carToPay ? [carToPay] : []),
    ...trainsToPay,
  ...
  const alreadyPaidItems: (FlightBooking | HotelBooking | CarRental | TrainBooking)[] = [ 
  ...
  ].filter(Boolean) as (FlightBooking | HotelBooking | CarRental | TrainBooking)[];
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (!allSucceeded && paymentError && !trainsToPay.length) throw new Error(paymentError); 
  ...
  const renderItemSummary = (item: FlightBooking | HotelBooking | CarRental | TrainBooking, typeLabel: string, isPaid: boolean = false) => {
    let title = "";
    ...
    else if ('trainLine' in item) title = `${item.trainLine} ${(item as TrainBooking).trainNumber || ''}`; // Cast to TrainBooking to access trainNumber
        ...
        <div className="mt-8 sm:mt-10 pt-6 border-t border-slate-200 dark:border-slate-700 flex flex-col-reverse sm:flex-row sm:justify-between gap-3">
          <button
            onClick={() => {
                ...
                if (currentTrip.trains && currentTrip.trains.some(t => t.booked && !t.paymentCompleted)) {
