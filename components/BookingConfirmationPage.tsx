...
interface CrossSellContextType {
  originalSource: 'standalone_flight' | 'standalone_hotel' | 'standalone_car';
  destination: string;
  dates?: string;
}
...
interface BookingConfirmationPageProps {
  ...
  onPlanFullTripFromCrossSell: (destination: string, dates?: string) => void; // New prop
  crossSellContext: CrossSellContextType | null;
}
...
const BookingConfirmationPage: React.FC<BookingConfirmationPageProps> = ({
  ...
  onPlanFullTripFromCrossSell, // Destructure new prop
  crossSellContext,
}) => {
  ...
  if (!currentTrip) {
    return (
      <div className="max-w-3xl mx-auto p-6 text-center">
        ...
        <button onClick={onSkipCrossSellAndGoHome} className="mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
          Go to Home
        </button>
      </div>
    );
  ...
  const SuggestionCard: React.FC<{ suggestion: MockSuggestion, onBook: () => void, icon: React.ReactNode, itemType: string }> = ({ suggestion, onBook, icon, itemType }) => (
    <div className="bg-slate-100 dark:bg-slate-700 p-4 rounded-lg shadow-md border border-slate-200 dark:border-slate-600 flex flex-col">
      ...
      <button onClick={onBook} className="mt-auto w-full bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium py-2 px-3 rounded-md transition-colors">
        Book {itemType}
      </button>
    </div>
  );
          ...
          {/* New "Plan Full Trip" button */}
           <div className="mt-6 text-center">
                <button
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center px-4 sm:px-6 py-2.5 sm:py-3 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-lg shadow-md transition-colors transform hover:scale-105"
                >
      ...
      <div className="text-center pt-6 border-t border-slate-200 dark:border-slate-700">
        {isStandaloneBooking ? (
            <button
                ...
                className="w-full sm:w-auto px-6 sm:px-8 py-2.5 sm:py-3 bg-slate-600 hover:bg-slate-700 text-white font-semibold rounded-lg shadow-md transition-colors"
            >
                Finish & Go Home
            </button>
        ) : (
            <button
                ...
                className="w-full sm:w-auto px-6 sm:px-8 py-2.5 sm:py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg shadow-md transition-colors transform hover:scale-105"
            >
